...
#define INPUT_CHUNK_HEIGHT_PX       1
...
//#define OUTPUT_FB_0                 (XPAR_MIG7SERIES_0_BASEADDR + BUF_ADDR_OFFSET)
//#define OUTPUT_FB_1                 OUTPUT_FB_0
//
//#define INPUT_FB_0                  OUTPUT_FB_0
//#define INPUT_FB_1                  OUTPUT_FB_0
...
//     setup_dma(&Dma[3], &DmaConfig[3], (UINTPTR) XPAR_AXI_DMA_3_BASEADDR);
  ...
  while (1) {
//      for (int r = 0; r < OUTPUT_FRAME_HEIGHT; r++) {
//        for (int c = 0; c < OUTPUT_FRAME_WIDTH; c++) {
//          char* pixel_base_1 = (char*) OUTPUT_FB_0 + 3 * (r * OUTPUT_FRAME_WIDTH + c);
//          char* pixel_base_2 = (char*) OUTPUT_FB_1 + 3 * (r * OUTPUT_FRAME_WIDTH + c);
//
//          *(pixel_base_1 + 0) = 0x00;
//          *(pixel_base_1 + 1) = 0xFF;
//          *(pixel_base_1 + 2) = 0xFF;
...
//        }
        ...
        write_chunk_parallel(Dma, OUTPUT_FB_0, input_row, input_col, INPUT_CHUNK_WIDTH_PX, INPUT_CHUNK_HEIGHT_PX);
//        write_chunk(Dma, OUTPUT_FB_0, input_row, input_col, INPUT_CHUNK_WIDTH_PX, INPUT_CHUNK_HEIGHT_PX);
//        chunk_ctr = 0;
//        write_chunk_parallel(Dma, INPUT_FB_0, output_row, output_col, INPUT_CHUNK_WIDTH_PX, INPUT_CHUNK_HEIGHT_PX);
        ...
        chunk_ctr++;
        if (chunk_ctr >= NUM_CHUNKS_PER_FRAME) {
            chunk_ctr = 0;
        }
  }
}
...
void fetch_chunk_parallel(XAxiDma *dma, u32 fb_base_addr, u32 row, u32 col, u32 chunk_width, u32 chunk_height) {
    ...
    addr = fb_base_addr + row * INPUT_FRAME_WIDTH * PIXEL_SIZE_BYTE + col * PIXEL_SIZE_BYTE;
    for (u32 i = 0; i < chunk_height; i++) {
//    dma_super_simple_transfer_to(&dma[0], addr, chunk_width * PIXEL_SIZE_BYTE);
      XAxiDma_SimpleTransfer(&dma[0], addr, chunk_width * PIXEL_SIZE_BYTE, XAXIDMA_DMA_TO_DEVICE);
    addr += INPUT_CHUNK_WIDTH_PX * PIXEL_SIZE_BYTE;
    }
}
...
void write_chunk_parallel(XAxiDma *dma, u32 fb_base_addr, u32 row, u32 col, u32 chunk_width, u32 chunk_height) {
    ...
    addr = fb_base_addr + row * INPUT_FRAME_WIDTH * PIXEL_SIZE_BYTE + col * PIXEL_SIZE_BYTE;
    for (u32 i = 0; i < chunk_height; i++) {
    // dma_super_simple_transfer_from(&dma[0], addr, chunk_width * PIXEL_SIZE_BYTE);
      XAxiDma_SimpleTransfer(&dma[0], addr, chunk_width * PIXEL_SIZE_BYTE, XAXIDMA_DEVICE_TO_DMA);
//      XAxiDma_SimpleTransfer(&dma[0], addr, 5, XAXIDMA_DEVICE_TO_DMA);
    addr += INPUT_CHUNK_WIDTH_PX * PIXEL_SIZE_BYTE;
    }
}
