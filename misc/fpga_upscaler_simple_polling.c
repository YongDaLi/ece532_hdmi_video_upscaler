...
#define INPUT_CHUNK_WIDTH_PX        100
...
#define OUTPUT_FB_1                 (XPAR_MIG7SERIES_0_BASEADDR + BUF_ADDR_OFFSET + OUTPUT_FRAME_WIDTH * OUTPUT_FRAME_HEIGHT * PIXEL_SIZE_BYTE)  // already aligned to 1024 address

#define INPUT_FB_0                  (OUTPUT_FB_1 + OUTPUT_FRAME_WIDTH * OUTPUT_FRAME_HEIGHT * PIXEL_SIZE_BYTE)
#define INPUT_FB_1                  (INPUT_FB_0 + INPUT_FRAME_WIDTH * INPUT_FRAME_HEIGHT * PIXEL_SIZE_BYTE) // for storing last input frame
...
int main(){
	...
	// setup DMA
	setup_dma(&Dma0, &Dma0Config, (UINTPTR) XPAR_AXI_DMA_0_BASEADDR);
    setup_dma(&Dma1, &Dma1Config, (UINTPTR) XPAR_AXI_DMA_1_BASEADDR);
	...
	while (1) {
        ...
        fetch_chunk(&Dma1, INPUT_FB_1, input_row, input_col, INPUT_CHUNK_WIDTH_PX, INPUT_CHUNK_HEIGHT_PX);

        // wait for processing results
        usleep(10);

        // writh chunk from processing to DDR
...
void setup_vdma() {
    ...
    vdma_membase[0x60 / sizeof(uint32_t)] = OUTPUT_FB_1;  // second frame buffer

    /* MM2S_FRMDLY_STRIDE */
    vdma_membase[0x58 / sizeof(uint32_t)] = OUTPUT_FRAME_WIDTH * PIXEL_SIZE_BYTE;
    // MM2S_HSIZE (in bytes)
    vdma_membase[0x54 / sizeof(uint32_t)] = OUTPUT_FRAME_WIDTH * PIXEL_SIZE_BYTE;
    ...
    vdma_membase[0x50 / sizeof(uint32_t)] = OUTPUT_FRAME_HEIGHT;
